...
class DarknetExtractor(chainer.ChainList):
    ...
    def __init__(self):
        ...
        # Darknet
        for k in range(7):
            self.append(Conv2DBNActiv(16 << k, 3, pad=1, activ=yolo_v2._leaky_relu))

        # additional link
        self.append(Conv2DBNActiv(1024, 3, pad=1, activ=yolo_v2._leaky_relu))
    ...
    def __call__(self, x):
        ...
        for i, link in enumerate(self):
            h = link(h)
            if i < 5:
                h = yolo_v2._maxpool(h, 2)
            elif i == 5:
                h = yolo_v2._maxpool(h, 2, stride=1)
        return h


class YOLOv2Tiny(yolo_v2.YOLOv2Base):
    """YOLOv2 tiny.
